...
import com.scality.keycloak.truststore.TruststoreProviderSingleton;
...
/**
 * Using this class is ugly, but it is the only way to push our truststore to
 * the default LDAP client implementation.
 * <p>
 * This SSLSocketFactory can only use truststore configured by
 * TruststoreProvider after the ProviderFactory was
 * initialized using standard Spi load / init mechanism. That will only happen
 * if "truststore" provider is configured
 * by the Keycloak Provider SPI configuration mechanism
 * <p>
 * If TruststoreProvider is not available this SSLSocketFactory will delegate
 * all operations to the SSLSocketFactory
 * returned by
 * {@link org.keycloak.common.crypto.CryptoProvider#wrapFactoryForTruststore(javax.net.ssl.SSLSocketFactory)},
 * which will delegate further to the factory returned by
...
public class SSLSocketFactory extends javax.net.ssl.SSLSocketFactory implements Comparator {
    ...
    public static synchronized SSLSocketFactory getDefault() {
        // Force recreation of the SSL factory to ensure it is using the latest
        // truststore
