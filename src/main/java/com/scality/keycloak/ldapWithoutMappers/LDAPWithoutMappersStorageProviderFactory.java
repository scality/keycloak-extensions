...
public class LDAPWithoutMappersStorageProviderFactory extends LDAPStorageProviderFactory {
    ...
    // enough for ResilientLDAPStorageProvider to convert it into a generic login error, rather
    // than hanging until the proxy renders a 502. See RING-54200.
    ...
    public LDAPStorageProvider create(KeycloakSession session, ComponentModel model) {
        // Reuse the LDAPIdentityStore built by the stock factory (its registry field is private),
        // then wrap it in the outage-resilient provider.
        LDAPStorageProvider stockProvider = super.create(session, model);
        return new ResilientLDAPStorageProvider(this, session, model, stockProvider.getLdapIdentityStore());
    }
