...
public class LDAPWithoutMappersStorageProviderFactory extends LDAPStorageProviderFactory {
    ...
    // Keycloak treats an empty timeout as "wait forever", so a connect to an unreachable LDAP
    // server hangs until the reverse proxy's read timeout fires and renders a 502 Proxy Error.
    // Bounding them well under that proxy timeout makes the failure surface quickly as a normal
    // error page instead. See RING-54200.
